...
// Categorías ordenadas por prioridad del negocio
const CATEGORIES = [
  'PROTEINAS',
  'CREATINAS',
  'PRE ENTRENOS',
...
export default function ProductPanel({ cart, onAdd, searchRef }: Props) {
    ...
    try {
      while (true) {
        const { data, error: dbError } = await supabase
          ...
          .select('*, product:products(id, name, department)')
  ...
  // Mapa de cantidad por variant_id para los badges
  ...
  const filtered = useMemo(() => {
    ...
    if (activeCategory) {
      list = list.filter(v =>
        (v.product?.department ?? '').toUpperCase() === activeCategory
      )
    }
    if (search.trim()) {
  ...
  function handleSearchKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    // Si no es barcode exacto → solo filtra (no agrega)
  }
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                  <span
                    className="text-xs"
                    style={{ color: variant.stock <= (variant.min_stock ?? 0) ? '#FF6B6B' : 'var(--text-muted)' }}
                  >
                    {variant.stock} pzs
                  </span>
